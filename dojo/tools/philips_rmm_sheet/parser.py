...
class PhilipsRMMSheetParser(object):
    ...
    def get_findings(self, filename, test):
        ...
        try:
            ...
            df = handler(filename, sheet_name="RMM", skiprows=2)
            ...
            return self._process_findings(df, test, mitigation_lookup)
    ...
    def _convert_vulnerability_to_finding(self, vulnerability, test):
        ...
        if vuln_desc:
            ...
            if desc_hash in self._title_cache:
                ...
                finding.title = title if title else (vuln_desc[:100] + '...' if len(vuln_desc) > 100 else vuln_desc)
...
Business Impact: {impact.get('business', {}).get('impact', '')} (Score: {impact.get('business', {}).get('score', '')})
        ...
        stride_tags_list = [value for value in stride_threats.values()]
