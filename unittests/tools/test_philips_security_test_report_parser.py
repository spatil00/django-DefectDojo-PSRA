...
class TestPhilipsSecurityTestReportParser(TestCase):
    ...
    def test_philips_security_test_report_parser_with_no_vuln_has_no_findings(self):
        testfile = open("unittests/scans/philips_security_test_report/philips_security_test_report_zero_vul.json", encoding="utf-8")
    ...
    def test_philips_security_test_report_parser_with_one_criticle_vuln_has_one_findings(self):
        testfile = open("unittests/scans/philips_security_test_report/philips_security_test_report_one_vul.json", encoding="utf-8")
        parser = PhilipsSecurityTestReportParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_philips_security_test_report_parser_with_many_vuln_has_many_findings(self):
        testfile = open("unittests/scans/philips_security_test_report/philips_security_test_report_many_vul.json", encoding="utf-8")
        parser = PhilipsSecurityTestReportParser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
        for finding in findings:
    ...
    def test_philips_security_test_report_parser_empty_with_error(self):
        with self.assertRaises(ValueError) as context:
            testfile = open("unittests/scans/philips_security_test_report/empty_with_error.json", encoding="utf-8")
            parser = PhilipsSecurityTestReportParser()
            findings = parser.get_findings(testfile, Test())
            testfile.close()
            for finding in findings:
